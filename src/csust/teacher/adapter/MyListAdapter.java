...
public class MyListAdapter extends BaseAdapter {
	...
	public MyListAdapter(NotificationManager mNotificationManager, Context ctx,
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			hold.btnDownload = (Button) convertView
					.findViewById(R.id.list_downloadData);
		...
		hold.btnDownload.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String myUrl = Model.GETSIGNEDREPORT + "course_id="
						+ list.get(position).getCourseName() + "&courseName="
	...
	Handler hand = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			if (msg.what == 404) {
				...
				if (result.equals("[0]")) {
					Toast.makeText(MyListAdapter.this.ctx, "抱歉，生成失败！", 1)
							.show();
				} else {
					Toast.makeText(MyListAdapter.this.ctx, "生成数据报表文件成功！", 1)
							.show();
					// 进入下载页面，直接进行下载，利用notification
					// 修改后，不必进入下载页面，直接给个notification，然后后台下载，从另一个入口查看下载消息。
					String fileName = Model.MYUSERINFO.getTeacher_name() + "-"
							+ Model.MYUSERINFO.getTeacher_username() + "-"
							+ result.split(",")[1] + System.currentTimeMillis();
					beginDownload(Model.USERREPORTURL + result.split(",")[0],
							fileName);
				}
			}
		};
	...
	private void beginDownload(String url, String fileName) {
		...
		try {
			// 直接在后台下载。
			downloadManager.addNewDownload(url, fileName, target, true, true,
					new DownloadRequestCallBack());
		} catch (DbException e) {
			e.printStackTrace();
		}

		// MyFileDownLoader myDownloader = new MyFileDownLoader(ctx,
		// mNotificationManager, url);
