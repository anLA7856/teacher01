...
public class DownloadManager {
	 ...
	 *            文件名
	 * @param target
	 *            保存路径 示例：String target = "/sdcard/xUtils/" + "lzfile.apk";
	 * @param autoResume
	 ...
	 * @throws DbException
	 */
	public synchronized void addNewDownload(String url, String fileName,
	 ...
	 * @return
	 */
	private PendingIntent getPendingIntent(boolean isDownloadFinish,
	...
	private void showCustomProgressNotify(DownloadInfo downloadInfo) {
		...
		if (downloadInfo.getProgress() != 0) {
			if (downloadInfo.getFileLength() != 0) {
				progress = (int) (downloadInfo.getProgress() * 100 / downloadInfo
						.getFileLength());
			}
		}
		if (progress >= 100) {
	...
	public class ManagerCallBack extends RequestCallBack<File> {
		...
		@Override
		public synchronized void onLoading(long total, long current,
				boolean isUploading) {
			HttpHandler<File> handler = downloadInfo.getHandler();
			if (handler != null) {
				downloadInfo.setState(handler.getState());
